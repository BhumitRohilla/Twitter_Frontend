...
export default function Home(){
    ...
    const [imgUrlArr,changeImgUrl] = useState([]);
    ...
    function handleFile(ev){
        if((ev.target.files.length + imgUrlArr.length) > 4){
            alert("Only four images are allowed");
        }else{
            console.log(ev.target.files);
            let url = [];
            for( let i = 0; i<ev.target.files.length ; ++i){
                url.push(URL.createObjectURL(ev.target.files[i]));
            }
            changeImgUrl([...imgUrlArr,...url]);
        }
    }
                ...
                <>
                    <div className={Styles.header}>
                        ...
                        <div className={Styles.buttonCluster}>
                            <div className={Styles.topButton}>For You</div>
                            <div className={Styles.topButton}>Following</div>
                        </div>
                    </div>
                    <div className={Styles.tweetCreation}>
                        <div className={Styles.profileHolder}>
                            <div className={Styles.profile}></div>
                        </div>
                        <div className={Styles.mainHolder}>
                            <div className={Styles.tweetInputHolder}>
                                <textarea maxLength={300} className={Styles.tweetInput} onChange={handleInput} value={tweet} type="text" placeholder="What's happening?" />
                                {/* <span className={Styles.tweetInput} onInput={handleInput} contentEditable={true}></span> */}
                            </div>
                            <div className={Styles.imageHolder}>
                                {
                                    imgUrlArr.map((element)=>{
                                        console.log(element);
                                        return <img className={Styles.tweetImage} src={element}/>
                                    })
                                }
                            </div>
